...
**Data pipeline:** A GitHub Action checks the most recent 365 days of USDA MyMarketNews report 3195 every weekday and on manual request. This schedule is more frequent than the bi-weekly report, allowing new editions and USDA corrections to be captured promptly. The repository owner stores a personal `USDA_MMN_API_KEY` as an Actions Secret. The key is used only in the Action and is not sent to visitors or stored in the site. `scripts/normalize_usda.py` validates the report section, product class, Illinois location, asking-price basis, unit, distributor category, final report status, and average price before publishing `data/prices.json`. If a validated value changes, the Action commits the data and rebuilt offline snapshot; GitHub Pages then redeploys the public site. The raw API response is retained as a one-day Actions artifact and is excluded from the public repository.
